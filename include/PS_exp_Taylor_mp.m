function [p, s, Cp, di_vec, time] = PS_exp_Taylor_mp(X, m, u, delta, timing)
...
if nargin > 4 && timing
   time = tic;  % the runtime of the algorithm
end
...
coef_expTaylor = 1./factorial(mp((0:m)))';
...
if coef_mat_index<r
    % form B_i(X), i=1:r-1
    q = coef_expTaylor(s*coef_mat_index+1)*Xsq_powers(:,:,1);
    for i=1:s-1
        q = q + coef_expTaylor(s*coef_mat_index+i+1)*Xsq_powers(:,:,i+1);
    end
else
    % form B_r(X), num_term = m-s*r;
    q = coef_expTaylor(s*r+1)*Xsq_powers(:,:,1);
    for i=1:m-s*r
        q = q + coef_expTaylor(s*r+i+1)*Xsq_powers(:,:,i+1);
    end
end
end
