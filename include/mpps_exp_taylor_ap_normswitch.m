function [p, s, cr, divec, time] = mpps_exp_taylor_ap_normswitch(X, m, u)
% PS_EXP_TAYLOR_AP_NORMSWITCH    The variant of PS_EXP_TAYLOR_AP that
% checks the norm of the input marix and can set the parameter s in the PS
% scheme to be larger than ceil(sqrt(m)).
%
% See the description of PS_EXP_TAYLOR_AP for details.
...
if normX<=s/e
    ...
    for j=1:r
        ...
        usq_prec(j+1) = Bsq_coefmat_norm(1)*u/(Bsq_coefmat_norm(j+1)*mp(normY)^(j));
    end
    % fprintf('small norm case... \n');
else
    r = floor(m/s);
    nu = r+1;
    Bsq_coefmat = zeros(n,n,r+1,'mp');
