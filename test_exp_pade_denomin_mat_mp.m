...
while i<=num_total_mats
    ...
    X = A/2^scal;
    n_mats(ii) = size(X,1);
    coef = comput_coef_Pade(m, num_digs, 'denominator');
...
% save the data
dataname = sprintf('data/exp_pade_denomin_mat_mp.mat');
...
exportgraphics(gca, '../figs/err-mat_n100_u64_gen_pade_denomin.eps', 'ContentType', 'vector');
...
exportgraphics(gca, '../figs/cmplxreduc-mat_n100_u64_gen_pade_denomin.eps', 'ContentType', 'vector');
