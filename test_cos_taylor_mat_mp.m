...
while i<=num_total_mats
    % fprintf('\n* Matrix id: %d\n', i);
    A = testmats(i,n);
    [~, scal, m] = cosm_mp(A,'algorithm','transfree','maxdegree',round(num_digs*1.5));
    while m==0 % exclude the 2-by-2 nilpotent case
        i = i + 1;
        A = testmats(i,n);
        [~, scal, m] = cosm_mp(A,'algorithm','transfree','maxdegree',round(num_digs*1.5));
    end
    X = A/2^scal;
    X = X * X; % PS for the cosine works with X^2
    m = ceil(m/2);
    n_mats(ii) = size(X,1);
...
% save the data
dataname = sprintf('data/cos_taylor_mat_mp.mat');
...
xlim([0,num_mats+1]);
xticks([0:15:75 num_mats]);
ylim([1e-65 1e-59])
yticks(10.^(-65:1:-59))

exportgraphics(gca, '../figs/err-mat_n100_u64_gen_cos.eps', 'ContentType', 'vector');
...
exportgraphics(gca, '../figs/cmplxreduc-mat_n100_u64_gen_cos.eps', 'ContentType', 'vector');
