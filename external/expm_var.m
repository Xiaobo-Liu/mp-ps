...
if size(A,1) ~= size(A,2)
     error(message('MATLAB:expm:inputMustBeSquare'));
end
...
if isdiag(A) % Check if A is diagonal.
    F = diag(exp(full(diag(A))));
    return
end
...
T = A;
recomputeDiags = matlab.internal.math.isschur(A);
...
if s ~= 0
    T = T./(2.^s);
   for ii = 1:length(Tpowers)
       Tpowers{ii} = Tpowers{ii}./(2.^(s.*ii));
   end
end
...
F = matlab.internal.math.nowarn.mldivide(V-U,2.*U) + I;  %F = (-U+V)\(U+V);
end
